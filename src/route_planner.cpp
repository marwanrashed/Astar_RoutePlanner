...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
 current_node->FindNeighbors();
  for (auto NB : current_node->neighbors) {
  	NB->h_value = this->CalculateHValue(NB);
...
RouteModel::Node *Iterator = current_node;
  while (Iterator != nullptr) {
  	path_found.push_back(*Iterator);
    if (Iterator->parent) this->distance += Iterator->distance(*(Iterator-> parent));
